...
cdef class ModifyOrderList(TradingCommand):
    """
    Represents a command to modify a list of orders in bulk.
    ...
    strategy_id : StrategyId
        The strategy ID for the command.
    instrument_id : InstrumentId
        The instrument ID associated with the orders.
    modify_orders : list[ModifyOrder]
        The list of modify order commands to execute.
    command_id : UUID4
        The command ID.
    ts_init : uint64_t
        UNIX timestamp (nanoseconds) when the object was initialized.
    ...
    def __str__(self) -> str:
        return (
            f"{type(self).__name__}("
            f"modify_orders_count={len(self.modify_orders)})"
        )
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"strategy_id={self.strategy_id.to_str()}, "
            f"instrument_id={self.instrument_id.to_str()}, "
            f"modify_orders={self.modify_orders}, "
            f"command_id={self.id.to_str()}, "
            f"correlation_id={self.correlation_id.to_str() if self.correlation_id is not None else None}, "
            f"ts_init={self.ts_init})"
        )
    ...
    @staticmethod
    cdef ModifyOrderList from_dict_c(dict values):
        Condition.not_none(values, "values")
        cdef str c = values["client_id"]
        cdef str corr = values.get("correlation_id")
        cdef list m_orders = [ModifyOrder.from_dict_c(m_dict) for m_dict in values["modify_orders"]]
        return ModifyOrderList(
            client_id=ClientId(c) if c is not None else None,
            trader_id=TraderId(values["trader_id"]),
            strategy_id=StrategyId(values["strategy_id"]),
            instrument_id=InstrumentId(values["instrument_id"]),
            modify_orders=m_orders,
            command_id=UUID4.from_str_c(values["command_id"]),
            ts_init=values["ts_init"],
            params=values.get("params"),
            correlation_id=UUID4.from_str_c(corr) if corr is not None else None,
    ...
    @staticmethod
    cdef dict to_dict_c(ModifyOrderList obj):
        Condition.not_none(obj, "obj")
        cdef ModifyOrder m
        return {
            "type": "ModifyOrderList",
            "client_id": obj.client_id.to_str() if obj.client_id is not None else None,
            "trader_id": obj.trader_id.to_str(),
            "strategy_id": obj.strategy_id.to_str(),
            "instrument_id": obj.instrument_id.to_str(),
            "modify_orders": [ModifyOrder.to_dict_c(m) for m in obj.modify_orders],
            "command_id": obj.id.to_str(),
            "ts_init": obj.ts_init,
            "params": obj.params,
            "correlation_id": obj.correlation_id.to_str() if obj.correlation_id is not None else None,
    ...
    @staticmethod
    def from_dict(dict values) -> ModifyOrderList:
        return ModifyOrderList.from_dict_c(values)

    @staticmethod
    def to_dict(ModifyOrderList obj):
        return ModifyOrderList.to_dict_c(obj)
